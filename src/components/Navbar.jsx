...
const Navbar = () => {
  ...
  return (
    <div className='max-w-[2000px] mx-auto flex justify-between items-center p-4 fixed z-10 bg-white w-full'>
		{/* Left side */}
		<div className='flex items-center'>
			<div onClick={()=> setNav(!nav)} className='cursor-pointer'>
				<AiOutlineMenu size={30}/>
			</div>
			<h1 className='flex text-2xl sm:text-3xl lg:text-4xl px-5'>
				Grub <span className='font-bold'>Spot</span>
			</h1>
		</div>
		{/* Search input */}
		...
		<div className={nav ? 'fixed top-0 left-0 w-[300px] h-screen bg-white z-10 duration-300' : 'fixed top-0 left-[-100%] w-[300px] h-screen bg-white z-10 duration-300'}>
			...
			<nav>
				<ul className='flex flex-col p-4 text-gray-800'>
					...
					<li className='text-xl py-4 flex cursor-pointer'><BsFillSaveFill size={25} className='mr-4' />Top Picks</li>
